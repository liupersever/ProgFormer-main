...
for index, (train, test) in enumerate(kf.split(X1, Y1)):
    ...
    netG = ProsMMA(1, [32, 64, 128], 8, 1, 30, 4, 2, 0.1).cuda()
    netG.apply(xavier_init_weights)
    netG.train()
    weight_decay = 5e-3
    optimizerG = optim.AdamW(netG.parameters(), lr=lr, betas=(0.999, 0.999), weight_decay=weight_decay)
    ...
    X_test = X1[test]
    Y_test = Y1[test]
    torch.save(X_test, f'X_{index}.pkl')
    score = 0.0
    acc_best = 0.0
    pre_best = 0.0
    recall_best = 0.0
